...
def path_trace(a:int, b:int, c:int, d:int):
    path = (c - a) ** 2 + (d - b) ** 2
    return path ** 0.5


a, b = map(int, input("Введіть координати першої точки (x1, y1): ").replace(", ", " ").split())
c, d = map(int, input("Введіть координати першої точки (x2, y2): ").replace(", ", " ").split())
path = path_trace(a, b, c, d)
print(path)
